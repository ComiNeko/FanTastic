...
<html>
<head>
<title>${creatorList[0].authorname}작가상세페이지</title>
...
<body>
	<div class="container">
		...
		<div class="creator-profile">
			<img src="${creatorList[0].authorimg1}"
				...
				<c:forEach var="product" items="${creatorList}">
					<c:if test="${product.productid != 0}">
						<div class="product-card">
							...
							<!-- ✅ 삭제 버튼: 로그인되어 있으면 보이도록 (비교 없이 무조건) -->
							<div style="margin-top: 10px;">
								<input type="button" value="삭제"
									onclick="if(confirm('정말 삭제하시겠습니까?')) { location.href='/post/productdelete.do?productid=${product.productid}'; }"
									style="color: white; background-color: red; padding: 5px 10px; border-radius: 5px; border: none; cursor: pointer;">
							</div>
						</div>
					</c:if>
				</c:forEach>
