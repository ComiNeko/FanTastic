...
public class PostDao {
	...
	public List<PostVo> getMyProductList(String userId) {
		...
		String sql = "SELECT * FROM NEW_PRODUCTS p " + "JOIN NEW_AUTHOR a ON p.authorid = a.authorid "
				+ "WHERE a.userid = ? " + "ORDER BY p.productid DESC";
	...
	public List<PostVo> getSelect() {
		...
		String sql = "select p.productid, p.productimage, a.authorname, c.categoryname, p.productname, p.productprice, p.productinfo \n" + 
				"from new_products p, new_author a, new_categories c\n" + 
				"where p.authorid = a.authorid and p.categoryid = c.categoryid\n" + 
	...
	public boolean isUserProductOwner(int productId, String userId) {
		...
		String sql = "SELECT COUNT(*) FROM NEW_PRODUCTS p " + "JOIN NEW_AUTHOR a ON p.authorid = a.authorid "
...
//		String sql = "SELECT * FROM ( " + " SELECT ROWNUM rnum, A.* FROM ( " + "     SELECT * FROM NEW_PRODUCTS "
//				+ (categoryId != -1 ? " WHERE categoryid = ? " : "") + "     ORDER BY productid DESC "
//				+ " ) A WHERE ROWNUM <= ? " + ") WHERE rnum >= ?";
		
		String sql = "select * " + 
				"from ( " + 
				"    select rownum rnum, product_infolist.* " + 
				"    from (\n" + 
				"        select p.productid, p.productimage, a.authorname, c.categoryname, p.productname, p.productprice, p.productinfo \n" + 
				"        from new_products p, new_author a, new_categories c\n" + 
				"        where p.authorid = a.authorid and p.categoryid = c.categoryid " + (categoryId != -1 ? " and p.categoryid = ? " : "") + 
				"        order by p.productid DESC " + 
				"    ) product_infolist  " + 
				"    where rownum <= ? " + 
				") " + 
	...
	public int getProductCountByCategory(int categoryId) {
		...
		String sql = "SELECT COUNT(*) FROM NEW_PRODUCTS " + (categoryId != -1 ? " WHERE categoryid = ?" : "");
...
//-------------------------------------------------------------//
	...
	public List<PostVo> getPostsByCategory(int categoryid) {
		...
		String sql = "select p.productid, p.productimage, a.authorname, c.categoryname, p.productname, p.productprice, p.productinfo \n" + 
				"from new_products p, new_author a, new_categories c\n" + 
				"where p.authorid = a.authorid and p.categoryid = c.categoryid and p.categoryid = 8 \n" + 
				"order by p.productid desc";
